  ...
  return (
    <div style={wrap}>
      ...
      <div style={grid}>
        {/* LEFT: Profil */}
        <div style={panel}>
          <SectionTitle icon="user" title="Profil Kamu" />
          ...
          <Field label="Pengalaman kerja / magang">
            <Textarea placeholder="e.g., Magang web developer 3 bulan di PT. ABC" value={form.experience} onChange={v => set('experience', v)} rows={3} />
          </Field>
          ...
          <SectionTitle icon="arrow_r" title="Target Lamaran" />
          <Field label="Posisi yang dilamar *">
            <Input placeholder="e.g., Frontend Developer, Data Analyst" value={form.targetJob} onChange={v => set('targetJob', v)} />
          </Field>
          ...
          <Field label="Deskripsi pekerjaan / konteks tambahan">
            <Textarea placeholder="e.g., posisi ini butuh pengalaman cloud computing, tim internasional" value={form.extraContext} onChange={v => set('extraContext', v)} rows={3} />
          </Field>
        </div>

        {/* RIGHT: Kustomisasi + Upload */}
        <div style={panel}>
          <SectionTitle icon="settings" title="Kustomisasi AI Coach" />
          <Field label="Persona AI coach">
            <Textarea
              placeholder="e.g., a strict but encouraging senior developer who gives honest feedback"
              value={form.aiPersona} onChange={v => set('aiPersona', v)} rows={3} />
          </Field>
          <Field label="Kesulitan kamu saat wawancara">
            <Textarea
              placeholder="e.g., sering ngeblank, susah menyusun kalimat bahasa Inggris, ingin contoh jawaban lebih baik"
              value={form.struggles} onChange={v => set('struggles', v)} rows={4} />
          </Field>

          <SectionTitle icon="clip" title="Upload Dokumen (Opsional)" />
      ...
      <div style={{ display:'flex',justifyContent:'center',padding:'24px 20px 36px' }}>
        <button style={startBtn(canStart)} disabled={!canStart} onClick={() => onStart(form)}>
          {canStart
            ...
            : <><Icon name="warning" size={16} color="var(--text3)" /> Isi nama &amp; posisi dulu</>
          }
        </button>
      </div>
    </div>
...
const SectionTitle = ({ icon, title }) => (
  <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:16,marginTop:8 }}>
    <Icon name={icon} size={15} color="var(--amber)" />
    <span style={{ fontFamily:'JetBrains Mono,monospace',fontSize:11.5,color:'var(--amber)',fontWeight:500,letterSpacing:'0.06em',textTransform:'uppercase' }}>{title}</span>
...
const heroTitle = { fontFamily:'Space Grotesk,sans-serif',fontSize:'clamp(26px,5vw,42px)',fontWeight:700,color:'var(--text)',lineHeight:1.2,marginBottom:12 }
...
const removeBtn = { background:'none',border:'none',cursor:'pointer',padding:'2px 4px',display:'flex',alignItems:'center' }
const startBtn  = ok => ({ display:'flex',alignItems:'center',gap:8,padding:'14px 40px',borderRadius:50,border:'none',background:ok?'linear-gradient(135deg,#f59e0b,#d97706)':'var(--bg3)',color:ok?'#1a0f00':'var(--text3)',fontSize:16,fontWeight:700,fontFamily:'Space Grotesk,sans-serif',cursor:ok?'pointer':'not-allowed',transition:'all .3s',boxShadow:ok?'0 4px 24px rgba(245,158,11,0.3)':'none' })
